...
static int playBack(snd_pcm_t *handle_p){
  ...
  while (1) {
    ...
    ret = recv(client_sock, (char *)&num_sets, sizeof(int), 0);
     fprintf(stderr, "\n Return receive numsets: %d", ret);
     fprintf(stderr,"\nRec numsets value: %d\n", num_sets);
    if(num_sets<0 || num_sets>128 ){
        fprintf(stderr,"\nNegative\n");
        buffer_s128_1[0]='1';
      }
    ...
    fprintf(stderr, "\n packet recieved length %d",strlen(buffer_s128_1));
    ...
    fprintf(stderr, "Return from send length:%d",lengthOfString);
    if (lengthOfString==0 || lengthOfString>128||lengthOfString<0){
        buffer_s128_2[0]='1';
        lengthOfString=strlen(buffer_s128_2);
        ret=send(client_sock, (char *)(&lengthOfString), sizeof(int), 0);   //This sends to the server the number of bytes that the client has and wishes to transfer.
        ret=send(client_sock, (char *)(buffer_s128_2), lengthOfString, 0);
    }
    else{
    ret=send(client_sock, (char *)(&lengthOfString), sizeof(int), 0);   //This sends to the server the number of bytes that the client has and wishes to transfer.
    ret=send(client_sock, (char *)(buffer_s128_2), lengthOfString, 0);
     fprintf(stderr, "\n packet sent of length: %d",strlen(buffer_s128_2));
   }
    //free(buffer_s128_2);
