...
static int playBack(snd_pcm_t *handle){
  ...
  while (count<RINGBUFFER_ROWS) {
   ...
   for(initial_rows_dma=RINGBUFFER_ROWS-CONST_DIST;initial_rows_dma<RINGBUFFER_ROWS;initial_rows_dma++){
    rc = snd_pcm_writei(handle, (&playback_array[initial_rows_dma][RINGBUFFER_COLUMNS]), frames);
    fprintf(stdout,"initial_rows_dma\n");
    }
  }
  #endif
