...
while(1){
  ...
  initial_rows_tcp=0;
    ...
    for(initial_rows_dma=0;initial_rows_dma<CONST_DIST; initial_rows_dma++)
    {
...
for(initial_rows_tcp=RINGBUFFER_ROWS-CONST_DIST;initial_rows_tcp<RINGBUFFER_ROWS; initial_rows_tcp++)
    {
      send(client_sock, (&record_array[initial_rows_tcp][RINGBUFFER_COLUMNS]), RINGBUFFER_COLUMNS, 0);
    }
